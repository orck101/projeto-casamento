...
const modalIconContainer = document.querySelector(".modal-gift-icon");
...
function renderGiftVisual(container, gift, modal = false) {
  ...
  if (isImagePath(gift.icon)) {
    ...
    image.src = gift.icon;
    image.alt = gift.name;
    image.className = modal ? "modal-gift-image" : "gift-image";
    image.loading = "lazy";
  ...
  use.setAttribute("href", `#${gift.icon || "icon-spark"}`);
...
function openGiftModal(gift, isFree = false) {
  ...
  renderGiftVisual(modalIconContainer, activeGift, true);
